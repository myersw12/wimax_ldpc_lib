...
int main(int argc, char *argv[])
{
    ...
    for (unsigned int i = 0; i < num_steps; i++)
    {
        ...
        // going to be
        for (unsigned int j = 0; j < 46080 / ber_test.get_codeword_len(); j++)
        {
        ...
        if(BER == 0.0)
        {
            ...
            // of bits required to qualify BER measurements.
            // In this case we are looking to go down to 
            // 1e-8 with a 98% confidence 
            // level.
        ...
        for (unsigned int j = 0; j < num_req/ber_test.get_codeword_len(); j++)
        {
            BER = ber_test.run_iteration(EbNo_dB);
        }
